...
Item
{
    ...
    Repeater
    {
        ...
        Rectangle
        {
            ...
            x: (docroot.count > 0 ? timeToPx(-docroot.offset) + index * pxOffset : 0) + (docroot.xOffset % pxOffset)

            property bool zero: (date.getHours() == 0) && (date.getMinutes() == 0)
            property date date:
            {
                var idx = lineRepeater.count - index -2 + (parseInt(docroot.xOffset / pxOffset))
